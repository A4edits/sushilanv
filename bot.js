...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please wait.')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('sushilan v3 𝚠𝚘𝚛𝚔𝚒𝚗𝚐 ' + config.WORKTYPE + ' 𝚗𝚘𝚠 👻'));
 ...
 //========================================end of reminder===========================================	    
		...
		try {
		            ...
		            if (error.message.includes('includes')) return;
                            if (config.LANG == 'ES') {
                                await conn.sendMessage(conn.user.jid, '*🤖  ERROR ANALYSIS [sushilan] *' + 
                                ...
                                await conn.sendMessage(conn.user.jid, '*🤖 ERROR ANALYSIS [sushilan] *' + 
                                    ...
                                    '\n_Aslo you can join our support group:_  https://chat.whatsapp.com/JhcYanAWH2G87NNHwPGmXD' +
                                    '\n_This message should have gone to your number (saved messages)._\n\n' +
                                    '*Error:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*🤖 ERROR ANALYSIS [sushilan] *' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '🙇🏻 Sorry, I Couldnt Read This Error!' +
                                        '\n_You can write to our support group for more help._  https://chat.whatsapp.com/JhcYanAWH2G87NNHwPGmXD '+'\n\n 🔻Reason might be \n'+ error
                                        , MessageType.text
                                    );
                                }    
                            }                      
